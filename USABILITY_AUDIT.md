...
**80 findings** · BROKEN 19 · DATA-GAP 1 · DATA-QUALITY 1 · DEAD-CONTROL 17 · DISAGREES-WITH-ITSELF 1 · MISLEADING 2 · NO-JOB 3 · SCAN2-GAP 21 · SLOW 15
...
## The state after the build (`34aa5d9`)

The register above is the **before** — the walk on `f2e1581` that produced
the build list. What follows is what the same walk found afterwards.

| | before (`f2e1581`) | after |
|---|---|---|
| `/dashboard/today` | **404** | 200 — the landing |
| `/dashboard/sales` | **404** | 200 — owner-only |
| pages carrying a nav | 0 | every one |
| metric-carrying elements | 21 | **64** |
| scan-2 keys compared | 13 | **39**, 0 divergences |
| the window buttons (7d…90d) | threw on brief, ads & sales, unit economics — 16 errored controls | **clean**, every renderer through its boundary |
| the landing's "Outflows & BAS" card | unclickable — the orb sat on it | **clickable** |
| the header's chat button on receivables | unclickable — the help button sat on it | **clickable** |
| show rate | landing 100% "verified" vs travelling 70% | **one rule** — 73% confirmed, range 73%–100% |
| "Cash net MTD" | $2,869.53, anchored to 15 Sep | **$1,118.84**, anchored to 1 Sep |

### Performance, measured cleanly

The browser figures in the table above were gathered with **four to six
Chromium instances hammering the same server at once**, so they measure
contention, not the app; they are not a fair before/after and are not
presented as one. These are single-client server timings, warm, best of two:

| page | budget | after |
|---|---|---|
| `/dashboard/today` | < 1000 ms | **766 ms** |
| `/ads/` | < 2000 ms | 645 ms |
| `/dashboard/landing` | < 2000 ms | 911 ms |
| `/dashboard/scale` | < 2000 ms | 959 ms |
| `/dashboard/view/brief` | < 2000 ms | 1026 ms |
| `/dashboard/view/cash` | < 2000 ms | 1048 ms |
| `/dashboard/view/projection` | < 2000 ms | 1047 ms |
| `/dashboard/view/team` | < 2000 ms | 1038 ms |
| `/dashboard/view/system` | < 2000 ms | 1041 ms |
| `/dashboard/sales` | < 2000 ms | 1544 ms |
| `/dashboard/scale/travelling` | < 2000 ms | **2518 ms — over budget** |

TODAY is the one page with a measured before *and* after by the same method:
**2971 ms → 766 ms**, after its trend series and ruling cards moved off the
request path onto the refresh loop.

### Stated exclusions

- **Scan 2 does not compare the legacy area pages' interiors.** Their
  numbers are built as HTML strings inside `dashboard.js` with no shared
  helper, so publishing the contract there is a rewrite, not an annotation.
  The metrics those pages *share* with TODAY and the landing — cash on hand,
  committed MRR, LTV:CAC, LTGP:CAC — do publish it and are compared.
- **The simulator is excluded on purpose.** Its numbers are a model. They
  publish `data-basis="scenario"`, and the scan keys on
  `(metric, window, basis)`, so a model can never be compared against a
  measurement.
- **65 write-capable controls were never activated.** READ-ONLY LAW #148.

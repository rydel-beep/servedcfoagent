...
def _opex_bridge(pdf, review):
    ...
    cur_lines = {li.get("name", "?"): abs(li.get("value") or 0) for li in (xr.get("opex_line_items") or [])}
    prior_lines = {li.get("name", "?"): abs(li.get("value") or 0) for li in (pxr.get("opex_line_items") or [])} if pxr.get("available") else {}
    # headline swing
